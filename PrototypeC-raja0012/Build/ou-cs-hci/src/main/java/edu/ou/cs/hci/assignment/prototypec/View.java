//******************************************************************************
// Copyright (C) 2019-2020 University of Oklahoma Board of Trustees.
//******************************************************************************
// Last modified: Sun Mar  1 12:41:45 2020 by Chris Weaver
//******************************************************************************
// Major Modification History:
//
// 20190203 [weaver]:	Original file.
// 20190220 [weaver]:	Adapted from swingmvc to fxmvc.
// 20200212 [weaver]:	Updated for new PrototypeB in Spring 2020.
// 20200228 [weaver]:	Added menu code for new PrototypeC in Spring 2020.
//
//******************************************************************************
//
//******************************************************************************

package edu.ou.cs.hci.assignment.prototypec;

//import java.lang.*;
import java.io.File;
import java.util.*;
import java.net.URL;
import javafx.event.*;
import javafx.scene.Scene;
import javafx.scene.control.*;
import javafx.scene.input.KeyCode;
import javafx.scene.input.KeyCodeCombination;
import javafx.scene.input.KeyCombination;
import javafx.scene.layout.*;
import javafx.scene.paint.Color;
import javafx.scene.shape.*;
import javafx.stage.*;
import edu.ou.cs.hci.assignment.prototypec.pane.*;

//******************************************************************************

/**
 * The <CODE>View</CODE> class.
 *
 * @author  Chris Weaver
 * @version %I%, %G%
 */
public final class View
{
	//**********************************************************************
	// Private Class Members (Layout)
	//**********************************************************************

	private static final double	SCENE_W = 960;		// Scene width
	private static final double	SCENE_H = 540;		// Scene height

	//**********************************************************************
	// Private Members
	//**********************************************************************

	// Master of the program, manager of the data, mediator of all updates
	private final Controller				controller;

	// TODO #2a: Add members for your menus/items here...
	private MenuItem						menuItemExample;
	//movies tab
	private MenuItem						aboutItem;
	private MenuItem						quitItem;
	//file tab
	private MenuItem						newItem;
	private MenuItem						openItem;
	private MenuItem						closeItem;
	private MenuItem						saveItem;
	private MenuItem						printItem;
	//edit tab
	private MenuItem						undoItem;
	private MenuItem						redoItem;
	private MenuItem						cutItem;
	private MenuItem						copyItem;
	private MenuItem						pasteItem;
	//window tab
	private MenuItem						collection1;
	private MenuItem						collection2;
	private MenuItem						collection3;
	private MenuItem						collection4;
	private MenuItem						collection5;
	private MenuItem						collection6;
	private MenuItem						collection7;


	// Handlers
	private final ActionHandler			actionHandler;
	private final WindowHandler			windowHandler;

	// Layout
	private final Stage					stage;
	private final ArrayList<AbstractPane>	panes;

	//**********************************************************************
	// Constructors and Finalizer
	//**********************************************************************

	// Construct a scene and display it in a window (stage).
	public View(Controller controller, String name, double x, double y)
	{
		this.controller = controller;

		// Create a listener for various widgets that emit ActionEvents
		actionHandler = new ActionHandler();

		// Create a listener to handle WINDOW_CLOSE_REQUESTs
		windowHandler = new WindowHandler();

		// Create a set of panes to include
		panes = new ArrayList<AbstractPane>();

		// Construct the pane for the scene.
		Pane	view = buildView();

		// Create a scene with an initial size, and attach a style sheet to it
		Scene		scene = new Scene(view, SCENE_W, SCENE_H);
		URL		url = View.class.getResource("View.css");
		String		surl = url.toExternalForm();

		scene.getStylesheets().add(surl);

		// Create a window/stage with a name and an initial position on screen
		stage = new Stage();

		stage.setOnHiding(windowHandler);
		stage.setScene(scene);
		stage.setTitle(name);
		stage.setX(x);
		stage.setY(y);
		stage.show();
	}

	//**********************************************************************
	// Public Methods (Controller)
	//**********************************************************************

	// The controller calls this method when it adds a view.
	// Set up the nodes in the view with data accessed through the controller.
	public void	initialize()
	{
		for (AbstractPane pane : panes)
			pane.initialize();

		// Initialize your menus/items here...
		menuItemExample.setOnAction(actionHandler);
		aboutItem.setOnAction(actionHandler);
		quitItem.setOnAction(actionHandler);
		newItem.setOnAction(actionHandler);
		openItem.setOnAction(actionHandler);
		closeItem.setOnAction(actionHandler);
		saveItem.setOnAction(actionHandler);
		printItem.setOnAction(actionHandler);
		undoItem.setOnAction(actionHandler);
		redoItem.setOnAction(actionHandler);
		cutItem.setOnAction(actionHandler);
		copyItem.setOnAction(actionHandler);
		pasteItem.setOnAction(actionHandler);
		collection1.setOnAction(actionHandler);
		collection2.setOnAction(actionHandler);
		collection3.setOnAction(actionHandler);
		collection4.setOnAction(actionHandler);
		collection5.setOnAction(actionHandler);
		collection6.setOnAction(actionHandler);
		collection7.setOnAction(actionHandler);
	}

	// The controller calls this method when it removes a view.
	// Unregister event and property listeners for the nodes in the view.
	public void	terminate()
	{
		for (AbstractPane pane : panes)
			pane.terminate();

		// Terminate your menus/items here...
		menuItemExample.setOnAction(null);
		aboutItem.setOnAction(null);
		quitItem.setOnAction(null);
		newItem.setOnAction(null);
		openItem.setOnAction(null);
		closeItem.setOnAction(null);
		saveItem.setOnAction(null);
		printItem.setOnAction(null);
		undoItem.setOnAction(null);
		redoItem.setOnAction(null);
		cutItem.setOnAction(null);
		copyItem.setOnAction(null);
		pasteItem.setOnAction(null);
		collection1.setOnAction(null);
		collection2.setOnAction(null);
		collection3.setOnAction(null);
		collection4.setOnAction(null);
		collection5.setOnAction(null);
		collection6.setOnAction(null);
		collection7.setOnAction(null);
	}

	// The controller calls this method whenever something changes in the model.
	// Update the nodes in the view to reflect the change.
	public void	update(String key, Object value)
	{
		for (AbstractPane pane : panes)
			pane.update(key, value);

		// Update your menus as needed when any old model properties change...
	}

	// The controller calls this method whenever something changes in the model.
	// Update the nodes in the view to reflect the change.
	public void	updateProperty(String key, Object newValue, Object oldValue)
	{
		for (AbstractPane pane : panes)
			pane.updateProperty(key, newValue, oldValue);

		// Update your menus as needed when any new model observables change...
	}

	//**********************************************************************
	// Private Methods (Layout)
	//**********************************************************************

	private Pane	buildView()
	{
		// OPTIONAL: Swap the comments for the pairs of panes below if you'd
		// like to see a reference solution to Prototype B. Swap them back to
		// work on the TODOs for Prototype C.

		//panes.add(new CollectionPaneB(controller));
		//panes.add(new EditorPaneB(controller));

		panes.add(new CollectionPane(controller));
		panes.add(new EditorPane(controller));

		// Create a tab pane with tabs for the set of included panes
		TabPane	tabPane = new TabPane();

		for (AbstractPane pane : panes)
			tabPane.getTabs().add(pane.createTab());

		MenuBar	menuBar = buildMenuBar();

		return new BorderPane(tabPane, menuBar, null, null, null);
	}

	//**********************************************************************
	// Inner Classes (Menus)
	//**********************************************************************

	private MenuBar	buildMenuBar()
	{
		MenuBar	menuBar = new MenuBar();

		// TODO #2b: Build your Menus and MenuBar below. For any MenuItems you
		// use, add members and code to initialize(), terminate(), update(), and
		// updateProperty() above, as needed. The following example code creates
		// a menu with one item in it, with examples of graphics for decoration.

		// Create some simple Nodes used as examples of graphics in menus/items.
		Rectangle	decoration1 = new Rectangle(8.0, 8.0, Color.RED);
		Circle		decoration2 = new Circle(4.0, Color.BLUE);

		// Create MenuItems...
		menuItemExample = new MenuItem("Item", decoration1);

		//Movies items
		aboutItem = new MenuItem("About");
		aboutItem.setAccelerator(new KeyCodeCombination(KeyCode.A, KeyCombination.CONTROL_DOWN));

		quitItem = new MenuItem("Quit");
		quitItem.setAccelerator(new KeyCodeCombination(KeyCode.Z, KeyCombination.CONTROL_DOWN));

		//File items
		newItem = new MenuItem("New");
		newItem.setAccelerator(new KeyCodeCombination(KeyCode.N, KeyCombination.CONTROL_DOWN));

		openItem = new MenuItem("Open");
		openItem.setAccelerator(new KeyCodeCombination(KeyCode.O, KeyCombination.CONTROL_DOWN));

		closeItem = new MenuItem("Close");
		closeItem.setAccelerator(new KeyCodeCombination(KeyCode.Q, KeyCombination.CONTROL_DOWN));

		saveItem = new MenuItem("Save");
		saveItem.setAccelerator(new KeyCodeCombination(KeyCode.S, KeyCombination.CONTROL_DOWN));

		printItem = new MenuItem("Print");
		printItem.setDisable(true);//disable print item in menu

		//Edit items
		undoItem = new MenuItem("Undo");//disable undo and redo
		undoItem.setDisable(true);

		redoItem = new MenuItem("Redo");
		redoItem.setDisable(true);

		cutItem = new MenuItem("Cut");
		cutItem.setAccelerator(new KeyCodeCombination(KeyCode.C, KeyCombination.CONTROL_DOWN));

		copyItem = new MenuItem("Copy");
		copyItem.setAccelerator(new KeyCodeCombination(KeyCode.X, KeyCombination.CONTROL_DOWN));

		pasteItem = new MenuItem("Paste");
		pasteItem.setAccelerator(new KeyCodeCombination(KeyCode.V, KeyCombination.CONTROL_DOWN));

		//Windows Items
		collection1 = new MenuItem("collection_1");
		collection2 = new MenuItem("collection_2");
		collection3 = new MenuItem("collection_3");
		collection4 = new MenuItem("collection_4");
		collection5 = new MenuItem("collection_5");
		collection6 = new MenuItem("collection_6");
		collection7 = new MenuItem("collection_7");

		// ...create Menus to hold them...
		Menu	menuExample = new Menu("Menu", decoration2);
		Menu	movies = new Menu("Movies");
		Menu	file = new Menu("File");
		Menu	edit = new Menu("Edit");
		Menu	window = new Menu("Window");

		// ...add the MenuItems to their menus...
		menuExample.getItems().addAll(menuItemExample);
		movies.getItems().addAll(aboutItem, quitItem);
		file.getItems().addAll(newItem, openItem, closeItem, saveItem, printItem);
		edit.getItems().addAll(undoItem, redoItem, cutItem, copyItem, pasteItem);
		window.getItems().addAll(collection1, collection2, collection3, collection4, collection5, collection6, collection7);

		//add separators to menus that need it
		SeparatorMenuItem fileSep1 = new SeparatorMenuItem();
		SeparatorMenuItem fileSep2 = new SeparatorMenuItem();
		SeparatorMenuItem editSep = new SeparatorMenuItem();
		file.getItems().add(1, fileSep1);//after new
		file.getItems().add(4, fileSep2);//after close
		edit.getItems().add(2, editSep);

		// ...then add the Menus to the MenuBar.
		menuBar.getMenus().addAll(movies, file, edit, window);

		return menuBar;
	}

	// TODO #3a: Implement the File/Open menu item handler, allowing the user
	// to select a CSV file to open. See the javafx.stage.FileChooser class.
	// Pass the chosen file to the Model via Controller.setProperty() to open.
	private void	handleFileOpenMenuItem()
	{
		FileChooser fileChooser = new FileChooser();
		fileChooser.setTitle("Select CSV file to Open");
		fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("CSV File", "*.csv"));
		File selectedFile = fileChooser.showOpenDialog(stage);
		String fileLocation = selectedFile.getAbsolutePath();
		controller.setProperty("file", fileLocation);

	}

	// TODO #3b: Implement the File/Save menu item handler, allowing the user
	// to select a CSV file to save. See the javafx.stage.FileChooser class.
	// Pass the chosen file to the Model via Controller.save() to save.
	private void	handleFileSaveMenuItem()
	{
		FileChooser fileChooser = new FileChooser();
		fileChooser.setTitle("Save CSV file");
		fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("CSV File", "*.csv"));
		File selectedFile = fileChooser.showSaveDialog(stage);
		controller.save(selectedFile);
	}

	//**********************************************************************
	// Inner Classes (Event Handlers)
	//**********************************************************************

	// TODO #2c: Add code to process user selection of each of your MenuItems.
	// Call your handleFileOpenMenuItem() and handleFileSaveMenuItem() methods
	// above for those two menu items. For all other menu items, print a brief
	// but informative message to the console.
	private final class ActionHandler
		implements EventHandler<ActionEvent>
	{
		public void	handle(ActionEvent e)
		{
			Object	source = e.getSource();

			if (source == menuItemExample)
				System.out.println("User selected the example menu item.");
			else if(source == aboutItem)
				System.out.println("User selected the About menu item");
			else if(source == quitItem)
				System.out.println("User selected the Quit menu item");
			else if(source == newItem)
				System.out.println("User selected the New menu item");

			//open
			else if(source == openItem) {
				System.out.println("User selected the Open menu item");
				handleFileOpenMenuItem();
			}
			//save
			else if(source == saveItem) {
				System.out.println("User selected the Save menu item");
				handleFileSaveMenuItem();
			}

			else if(source == closeItem)
				System.out.println("User selected the Close menu item");
			else if(source == printItem)
				System.out.println("User selected the Print menu item");
			else if(source == undoItem)
				System.out.println("User selected the Undo menu item");
			else if(source == redoItem)
				System.out.println("User selected the Redo menu item");
			else if(source == cutItem)
				System.out.println("User selected the Cut menu item");
			else if(source == copyItem)
				System.out.println("User selected the Copy menu item");
			else if(source == pasteItem)
				System.out.println("User selected the Paste menu item");
			else if(source == collection1)
				System.out.println("User selected the Collection1 menu item");
			else if(source == collection2)
				System.out.println("User selected the Collection2 menu item");
			else if(source == collection3)
				System.out.println("User selected the Collection3 menu item");
			else if(source == collection4)
				System.out.println("User selected the Collection4 menu item");
			else if(source == collection5)
				System.out.println("User selected the Collection5 menu item");
			else if(source == collection6)
				System.out.println("User selected the Collection6 menu item");
			else if(source == collection7)
				System.out.println("User selected the Collection7 menu item");
		}
	}

	private final class WindowHandler
		implements EventHandler<WindowEvent>
	{
		public void	handle(WindowEvent e)
		{
			if (e.getEventType() == WindowEvent.WINDOW_CLOSE_REQUEST)
				controller.removeView(View.this);
		}
	}
}

//******************************************************************************
