...
public final class CollectionPane extends AbstractPane
{
	...
	private TextField 				titleAccordion;
	private ListView<String>		genreAccordion;
	private ComboBox<String>		ratingAccordion;
	private TextField				yearAccordion;
	...
	private void	registerWidgetHandlers()
	{
		...
		titleAccordion.setOnAction(actionHandler);
		genreAccordion.getSelectionModel().getSelectedIndices().addListener(changeHandler);
		ratingAccordion.getSelectionModel().selectedItemProperty().addListener(this::changeString);
		yearAccordion.setOnAction(actionHandler);
	...
	private void	unregisterWidgetHandlers()
	{
		...
		titleAccordion.setOnAction(null);
		genreAccordion.getSelectionModel().getSelectedIndices().removeListener(changeHandler);
		ratingAccordion.getSelectionModel().selectedItemProperty().removeListener(this::changeString);
		yearAccordion.setOnAction(null);
	...
	private void	registerPropertyListeners(Movie movie)
	{
		...
		movie.yearProperty().addListener(this::handleChangeInteger);
		movie.ratingProperty().addListener(this::handleChangeItem);
		movie.runtimeProperty().addListener(this::handleChangeDecimal);

		movie.averageReviewScoreProperty().addListener(this::handleChangeDecimal);
		movie.genreProperty().addListener(this::handleChangeGenre);
	}
	...
	private void	unregisterPropertyListeners(Movie movie)
	{
		...
		movie.yearProperty().removeListener(this::handleChangeInteger);
		movie.ratingProperty().removeListener(this::handleChangeItem);
		movie.runtimeProperty().removeListener(this::handleChangeDecimal);

		movie.averageReviewScoreProperty().removeListener(this::handleChangeDecimal);
		movie.genreProperty().removeListener(this::handleChangeGenre);
	}
	...
	private Node	buildAccordion()
	{
		...
		titleAccordion = new TextField("Enter Title");

		//get the list from file, and add to genre listview
		List<String> gdata = (List<String>)controller.getProperty("genres");
		genreAccordion = new ListView<String>();
		genreAccordion.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE);
		genreAccordion.getItems().addAll(gdata);

		//get rating data and insert into ComboBox
		List<String> rdata = (List<String>)controller.getProperty("ratings");
		ratingAccordion = new ComboBox<String>();
		ratingAccordion.getItems().addAll(rdata);

		yearAccordion = new TextField("Enter Year");

		TitledPane titlePane = new TitledPane("Title", titleAccordion);
		TitledPane genrePane = new TitledPane("Genres", genreAccordion);
		TitledPane ratingPane = new TitledPane("Rating", ratingAccordion);
		TitledPane yearPane = new TitledPane("Year", yearAccordion);

		Accordion accordion = new Accordion();
		accordion.getPanes().addAll(titlePane, genrePane, ratingPane, yearPane);
		...
		implements Predicate<Movie>
	{
		public boolean	test(Movie movie)
		{
			...
			//rating
			String rating = ratingAccordion.getValue();
			String title = titleAccordion.getText();
			String year = yearAccordion.getText();

			//get the genres from accordion and movies genre and turn to string to filter it using contains
			List<String> genreSelected = genreAccordion.getSelectionModel().getSelectedItems();
			List<String> gdata = (List<String>)controller.getProperty("genres");
			String genreMovie = movie.getGenreAsString(gdata);

			//filter the genre, rating and year
			for(String s : genreSelected) {
				if (Objects.equals(movie.getRating(), rating) && String.valueOf(movie.getYear()).equals(year) && genreMovie.contains(s)) {
					return true;
				}
			}

			//if you are looking for a title, then the other values should match up too as it is aggresive search only
			for(String s : genreSelected) {
				if (Objects.equals(movie.getRating(), rating) && String.valueOf(movie.getYear()).equals(year) && genreMovie.contains(s) && movie.getTitle().equals(title)) {
					return true;
				}
			}
//
//			else if(movie.getTitle().equals(title) && movie.getRating().equals(rating) && (movie.getYear() == year)){
//				return true;
//			}
		...
		implements EventHandler<ActionEvent>
	{
		public void	handle(ActionEvent e)
		{
			...
			if(source == titleAccordion) {
				//titleAccordion.setText(source.toString());
				updateFilter();
			}
			if(source == yearAccordion) {
				updateFilter();
			}
	...
	// For rating, others are done in listeners
	private void	changeString(ObservableValue<? extends String> observable,
									 String oldValue, String newValue)
	{
			if (observable == ratingAccordion.getSelectionModel().selectedItemProperty()) {
				ratingAccordion.setValue(newValue);
				updateFilter();
			}
	}
	...
	private void	handleChangeDecimal(ObservableValue<? extends Number> observable,
										Number oldValue, Number newValue)
	{
		Movie	movie = (Movie)controller.getProperty("movie");

		String runtime =  (((Integer)newValue / 60) + "h " + ((Integer)newValue % 60) + "min");
	...
	//for Spinner<Integer>
	private void	handleChangeInteger(ObservableValue<? extends Number> observable,
										Number oldValue, Number newValue)
	{
		Movie	movie = (Movie)controller.getProperty("movie");

		if (observable == movie.yearProperty())
			summaryYear.setText(String.valueOf(newValue));
	...
	// For ComboBox
	private void	handleChangeItem(ObservableValue<? extends String> observable,
									 String oldValue, String newValue)
	{
		Movie	movie = (Movie)controller.getProperty("movie");
		if (observable == movie.ratingProperty())
			summaryRating.setText(String.valueOf(newValue));
	}

	//for Genre
	private void 	handleChangeGenre(ObservableValue<? extends Number> observable,
									  Number oldValue, Number newValue) {
		Movie movie = (Movie) controller.getProperty("movie");
		List<String> data = movie.getAllAttributesAsStrings();

		//the genre data is in index 11
		if (observable == movie.genreProperty()) {
			summaryGenre.setText(data.get(11));
			updateFilter();
		}
	}
	// For widgets with a numeric value like Slider and Spinner, you can't use
