...
public final class EditorPane2 extends AbstractPane
{
	...
	private void	buildWidgets()
	{
		...
		cSummary.setPrefColumnCount(53);
		cSummary.setPrefRowCount(5);
		cSummary.setWrapText(true);

		// Set up Comments TextArea to track text, caret, and selection actions.
		cComments = new TextArea();
		cComments.setPrefColumnCount(53);
		cComments.setPrefRowCount(4);
	...
	private Pane	buildLayout()
	{
		...
		//Directed By label
		Label lDirector = new Label("Directed by");
		lDirector.setFont(FONT_LABEL);
		lDirector.relocate(10, 55);
		cDirector.relocate(10, 70);
		cDirector.setPrefWidth(300);

		//isAnimated and isColor
		cIsAnimated.relocate(450, 70);
		cIsColor.relocate(380, 70);
		...
		lYear.relocate(10, 95);
		cYear.relocate(10, 115);

		Label lAverageScore = new Label("Average Score");
		lAverageScore.setFont(FONT_LABEL);
		lAverageScore.relocate(120, 95);
		cAverageReviewScore.relocate(120, 115);

		Label lNumberOfReviews = new Label("Number of Reviews");
		lNumberOfReviews.setFont(FONT_LABEL);
		lNumberOfReviews.relocate(220, 95);
		cNumberOfReviews.relocate(220, 115);

		Label lRating = new Label("Rating");
		lRating.setFont(FONT_LABEL);
		lRating.relocate(360, 95);
		cRating.relocate(360, 115);

		Label lRuntime = new Label("Runtime");
		lRuntime.setFont(FONT_LABEL);
		lRuntime.relocate(480, 95);
		cRuntime.relocate(480, 115);
		...
		lGenre.relocate(10, 150);
		cGenres.get(0).relocate(10, 170);
		cGenres.get(1).relocate(90, 170);
		cGenres.get(2).relocate(175, 170);
		cGenres.get(3).relocate(295, 170);
		cGenres.get(4).relocate(380, 170);
		cGenres.get(5).relocate(10, 205);
		cGenres.get(6).relocate(90, 205);
		cGenres.get(7).relocate(175, 205);
		cGenres.get(8).relocate(295, 205);
		cGenres.get(9).relocate(380, 205);
		...
		lAwards.relocate(10, 235);
		cAwardPicture.relocate(10, 250);
		cAwardDirecting.relocate(90, 250);
		cAwardCinematography.relocate(175, 250);
		cAwardActing.relocate(295, 250);
		...
		lSummary.relocate(10, 275);
		cSummary.relocate(10, 295);

		Label lComment = new Label("Comments");
		lComment.setFont(FONT_LABEL);
		lComment.relocate(10, 395);
		cComments.relocate(10, 410);

		//**************************************************************

		//Poster
		cImageView.relocate(670, 10);
		cImageFile.relocate(730, 450);
		cImageButton.relocate(670, 450);

		// Add all of the widgets, labels, decoration nodes, etc. to the pane
		pane.getChildren().addAll(lTitle, cTitle, lDirector, cDirector);
		pane.getChildren().addAll(cIsAnimated, cIsColor);
		pane.getChildren().addAll(lYear, cYear, lAverageScore, cAverageReviewScore, lNumberOfReviews, cNumberOfReviews, lRating, cRating, lRuntime, cRuntime);
		pane.getChildren().addAll(lGenre, cGenres.get(0), cGenres.get(1), cGenres.get(2), cGenres.get(3), cGenres.get(4), cGenres.get(5), cGenres.get(6), cGenres.get(7), cGenres.get(8), cGenres.get(9));
		pane.getChildren().addAll(lAwards, cAwardActing, cAwardCinematography, cAwardDirecting, cAwardPicture);
		pane.getChildren().addAll(lSummary, cSummary, lComment, cComments);
		pane.getChildren().addAll(cImageView, cImageButton, cImageFile);
