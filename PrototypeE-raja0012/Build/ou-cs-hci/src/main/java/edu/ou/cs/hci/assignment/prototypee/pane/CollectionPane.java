...
import edu.ou.cs.hci.assignment.prototypee.flow.CoverFlow;
import edu.ou.cs.hci.assignment.prototypee.fxml.SummaryFXML;
...
public final class CollectionPane extends AbstractPane
{
	...
	public void	updateProperty(String key, Object newValue, Object oldValue)
	{
		...
		coverflow.updateProperty(key, newValue, oldValue);
	}
	...
	private Pane	buildPane()
	{
		...
		Node	rregion = buildMovieViewUsingFXML();
	...
	{
		coverflow = new CoverFlow(controller);

		return coverflow.getBase();
	}

	private Node	buildMovieViewUsingFXML()
	{
		try
		{
			// Use a self-loading FXML class
			SummaryFXML	pane = new SummaryFXML();

			// TODO #01b: Copy over references to all widgets loaded from FXML.
		...
		{
			System.err.println("***  Exception loading SummaryFXML  ***");
			//System.err.println("*** Reverting to hard-coded version ***");
			ex.printStackTrace();

			Label	label = new Label("exception loading SummaryFXML");

			label.setPadding(PADDING);

			return label;
			//return buildMovieView();
		}
	}
